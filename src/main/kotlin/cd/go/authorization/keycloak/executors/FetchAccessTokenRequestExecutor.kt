...
class FetchAccessTokenRequestExecutor(
    private val request: FetchAccessTokenRequest
) : RequestExecutor {

    override fun execute(): GoPluginApiResponse {
        if (request.authConfigs().isNullOrEmpty()) {
            throw NoAuthorizationConfigurationException("[Get Access Token] No authorization configuration found.")
        }

        val configuration = request.authConfigs()!![0].configuration!!
        val tokenInfo = configuration.keycloakApiClient().fetchAccessToken(request.requestParameters())
